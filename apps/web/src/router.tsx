...
const RootRouteComponent = () => {
  ...
  if (!bootstrap) {
    return <div className="p-10 text-sm text-slate-500">Loading workspace shell...</div>;
  }
  ...
  return (
    ...
    >
      <AppShell
        ...
        duplicateTimesheetRow={() => setDuplicateSignal((value) => value + 1)}
      >
        <Outlet />
      </AppShell>
    </ShellContext.Provider>
...
const timesheetRoute = createRoute({
  ...
  component: () => {
    const shell = useShellState();
    const { data: bootstrap } = useQuery({ queryKey: ["bootstrap"], queryFn: api.bootstrap });
    return bootstrap ? <TimesheetPanel metadata={bootstrap.metadata} duplicateSignal={shell.duplicateSignal} /> : null;
  },
});
...
const settingsRoute = createRoute({
  ...
  component: () => {
    const { data: bootstrap } = useQuery({ queryKey: ["bootstrap"], queryFn: api.bootstrap });
    return bootstrap ? <SettingsPanel bootstrap={bootstrap} /> : null;
  },
});
