...
export async function GET(request: Request) {
  ...
  const timesheetUrl = new URL(`${backendApiBaseUrl}/timesheet`);

  for (const [key, value] of searchParams.entries()) {
    timesheetUrl.searchParams.set(key, value);
  }

  try {
    const response = await fetch(timesheetUrl, {
      cache: 'no-store',
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          message: payload?.message ?? `Timesheet request failed with ${response.status}`,
